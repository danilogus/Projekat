...
>
  ...
  <xsl:template match="/">
    <html>
      <body>
        <h3>Relevantne osobe i kompanije:</h3>
        <xsl:apply-templates/>
      </body>
    </html>
  </xsl:template>
  ...
  <xsl:template match="Adresar">
    ...
    <table border="1">
      ...
      <xsl:for-each select="Osoba">
        <xsl:if test="Adresa/@type='stalna'">
          <xsl:if test="DatumRodjenja/Godina &gt; 1990 ">
            <xsl:if test="DatumRodjenja/Godina &lt; 2005">
            <xsl:if test="PunoIme[starts-with(Prezime,'M')]">
              <tr>
                <td>
